...
impl<T: Clone + PartialEq + Eq + Default + std::fmt::Debug> List<T> for ArrayDeque<T> {
    ...
    fn get(&self, i: usize) -> Option<T> {
        Some(self.heap[(self.j + i) & (self.heap.len() - 1)].clone())
    }

    fn set(&mut self, i: usize, x: T) -> Option<T> {
        let previous = self.heap.get_mut((self.j + i) & (self.heap.len() - 1)).unwrap();
    ...
    fn add(&mut self, i: usize, x: T) {
        ...
        if i < self.n / 2 {
            ...
            for k in 0..i {
                let next = self.heap
                    .get_mut((self.j + k + 1) & (self.heap.len() - 1)).unwrap().clone();

                let current = self.heap.get_mut((self.j + k) & (self.heap.len() - 1)).unwrap();
                *current = next;
            }
        } else {
        ...
        let current = self.heap.get_mut((self.j + i) & (self.heap.len() - 1)).unwrap();
        *current = x.clone();
        self.n += 1;
    }

    fn remove(&mut self, i: usize) -> Option<T> {
        let x = self.heap.get((self.j + i) & (self.heap.len() - 1)).unwrap().clone();
        ...
        if i < self.n / 2 {
            for k in (1..i + 1).rev() {
                ...
                *current = previous;
                ...
                let previous = self.heap
                    .get_mut((self.j + k - 1) & (self.heap.len() - 1)).unwrap();
                *previous = T::default();
            }
            self.j = (self.j + 1) & (self.heap.len() - 1);
        } else {
            for k in i..self.n - 1 {
                let next = self.heap
                    .get_mut((self.j + k + 1) & (self.heap.len() - 1)).unwrap().clone();
...
impl<T: Clone + PartialEq + Eq + Default + std::fmt::Debug> ArrayDeque<T> {
    ...
    pub fn with_capacity(len: usize) -> Self {
        if len & (len - 1) != 0 {
            panic!("capacity should be power of 2");
        }
    ...
    fn resize(&mut self) {
        ...
        for k in 0..self.size() {
            let ai = self.heap.get((self.j + k) & (self.heap.len() - 1)).unwrap();
...
fn array_deque() {
    ...
    let mut a: ArrayDeque<char> = ArrayDeque::with_capacity(4);
    assert_eq!(a.size(), 0);
    a.add(0, 'a');
    a.add(1, 'b');
    a.add(2, 'c');
